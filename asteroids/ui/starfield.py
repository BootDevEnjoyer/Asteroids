...
class Star:
    """Individual star with position, speed, and visual properties for parallax layers."""
    ...
    def __init__(self, x, y, layer):
        ...
        if random.random() < 0.1:
            if random.random() < 0.5:
                self.color = (self.brightness, self.brightness // 2, self.brightness // 2)
            else:
                self.color = (self.brightness // 2, self.brightness // 2, self.brightness)

class Starfield:
    """Parallax starfield with multiple depth layers."""
    ...
    def add_twinkle_effect(self, screen):
        """Occasionally flash a random star to simulate twinkling."""
